...
| **부분 표본 뽑기를 최댓값 풀링으로 바꾼 편** | - | LeNet 노트 15절 둘 — 이 논문의 부분 표본에는 **학습되는 계수와 바이어스**가 있어 오늘날의 최댓값 풀링과 같은 것이 아니다. 지금 쓰는 망이 전부 최댓값 풀링을 쓰는데 그 자리가 언제 무엇 때문에 바뀌었는지를 모른다 | 편을 아직 못 골랐다 |
| **합성곱 망을 크게 키운 편** | - | LeNet 노트 13절 둘 — **파라미터가 많은 망이 왜 낮은 시험 오차를 내는지가 수수께끼로 남는다**고 논문이 스스로 적는다. 규모를 올렸을 때 논문의 설명(작은 가중치에서 용량이 서서히 큰다)이 그대로 서는지 본다 | 아직 |
