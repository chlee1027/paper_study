...
노트를 쓰며 「이건 직접 확인하고 싶다」가 된 것 셋을 적어 둔다. 아직 돌리지 않았다.

**하나. 0.25 표가 실제로 그 모양인가.** 3절의 표는 로지스틱 기울기의 최댓값을 곱한 것이라 상한이다. 실제 망에서 층마다 기울기가 몇 배씩 줄어드는지는 역전파 실험에서 **10~12배**로 쟀다(그 노트에 있다). **이 논문이 전제하는 문제의 크기를 같은 자리에서 재 두면** 두 편이 이어진다.

**둘. RBM 하나를 MNIST 에 배워 보는 것.** 대조 발산 $n = 1$ 로 500 유닛 RBM 하나를 배우고, 재구성한 이미지와 가중치 그림을 본다. 이 논문의 알맹이 중 **실제로 돌아가는 최소 단위**가 그것이다.

**셋. 탐욕 학습의 몫을 직접 가르는 것.** 같은 망을 (가) 무작위 초기화 + 미세조정, (나) 탐욕 초기화 + 미세조정 둘로 배워 오차를 견준다. **16절의 하나가 답이 없다고 적은 자리**이고, MNIST 라 지금 장비로 돌아간다.
