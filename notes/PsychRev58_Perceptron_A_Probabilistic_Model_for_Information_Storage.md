...
| 돌려 본 범위 | **2026년 9월 20일에 학습 규칙을 구현해 돌렸다.** 이 노트가 숫자 없이 적어 둔 한계 넷을 재는 실험이고, 본문에서 **「돌려 본 것」**으로 시작하는 문단이 그 결과다. **원문을 보고 맞춘 것이 아니라 표준형으로 알려진 규칙을 구현한 것이다** |
...
**돌려 본 것 — 한계는 지켜지지만 실제와 멀다.** 여유를 정해 뽑은 2차원 점 60개로 80번 돌렸더니 **한 번도 한계를 넘지 않았다.** 다만 그 한계가 실제보다 한참 크다.

| 여유 $\gamma$ | 한계 $(R/\gamma)^2$ | 실제로 틀린 횟수 (중앙값) | 최대 |
|---|---|---|---|
| 0.057 | 762 | **7** | 20 |
| 0.107 | 227 | **6** | 12 |
| 0.204 | 60 | **4** | 8 |
...
**이 정리는 「유한하다」를 보장할 뿐 횟수를 가늠하는 데 쓸 값이 아니다.** 여유가 가장 좁은 줄에서 한계가 762인데 실제로는 7번 틀렸다.
...
| **갈라지지 않으면 멈추지 않는다** | 수렴 정리의 조건이 깨지면 규칙이 계속 돈다. **언제 그만둘지를 모델이 알려 주지 않는다**. **돌려 보니 XOR 는 씨앗 50개 모두 상한 2,000 훑기까지 멈추지 않았고, 그동안 가중치를 중앙값 5,585번 고쳤다** — 멈추지 않으면서 계속 고치는 것이 이 한계의 모양이다 | 수렴 정리의 조건에서 바로 따라온다 |
| **어떤 선인지는 보장하지 않는다** | 갈라 놓기만 하면 멈추므로, 경계가 데이터에 바짝 붙어 있어도 그대로 끝난다. **돌려 보니 멈춘 자리의 여유가 그 데이터에서 얻을 수 있는 가장 큰 여유의 중앙값 0.537이었다** — 60번 모두 더 작았고, 가장 나쁜 줄은 0.005로 경계가 점에 거의 붙은 채 끝났다 | 뒤에 서포트 벡터 머신이 여유를 목표로 삼는다 |
...
- **"XOR를 못 푸는 것이 퍼셉트론이 실패한 이유"도 반만 맞다.** XOR는 **가장 작은 반례**일 뿐이다. 진짜 문제는 **표현할 수 있는 함수의 계열이 평평한 경계 하나로 묶여 있다**는 것이고, XOR는 그것을 네 점으로 보인 예시다.
  ...
  **돌려 본 것 — 네 점에서는 반례가 둘뿐이다.** 점 넷에 0과 1을 붙이는 방법 16가지를 전부 세어 보면 **14가지가 직선 하나로 갈리고, 갈리지 않는 것은 XOR와 그 반대(XNOR) 둘뿐**이다. 퍼셉트론 규칙도 정확히 그 14가지에서만 멈췄다. **「가장 작은 반례일 뿐」을 이 격자에서는 「16가지 중 둘」이라고 세어 적을 수 있다.** 다만 이것은 점이 넷일 때의 수이고, 점이 늘면 갈리는 비율은 빠르게 줄어든다 — 그 계산은 이번에 하지 않았다.
...
- **가중치를 0에서 시작하면 학습률 $\eta$ 는 아무것도 바꾸지 않는다.** 모든 갱신이 $\eta$ 배로 함께 커지므로 부호가 그대로이고, 부호만 판단에 쓰이기 때문이다. **돌려 보니 $\eta$ 를 0.01 · 0.1 · 1 · 10 으로 바꿔도 씨앗 15개 모두에서 가중치를 고친 횟수가 같았다.** 0이 아닌 곳에서 시작하면 15개 모두 달라진다.
...
**셋. 숫자 없이 적어 둔 한계는 돌려 보면 숫자가 붙는다.** 이 노트를 쓸 때 「경계가 바짝 붙어 있어도 그대로 끝난다」와 「XOR는 가장 작은 반례일 뿐」이라고 적었는데, 둘 다 세어 보니 **0.537과 「16가지 중 둘」**이 되었다. 읽어서 세운 문장과 재어서 세운 문장은 다음에 무엇을 할지까지 다르게 만든다.

**넷.** 지금 확인된 것은 여기까지다. **원문을 읽지 않았으므로 이 노트의 어떤 문장도 원문 인용이 아니고**, 돌려 본 것도 표준형으로 알려진 규칙을 구현한 것이지 원문의 절차를 옮긴 것이 아니다. 원문을 구하면 6절의 학습 규칙과 3절의 구조 서술을 먼저 맞춰 보아야 한다.
...
- 그림 일곱 개와 그것을 만드는 스크립트: `figures/PsychRev58_Perceptron/`
- **「돌려 본 것」 문단들의 근거**: `experiments/perceptron_1958/` — 노트북과 결과 CSV 다섯 개, 그리고 돌리기 전에 적은 질문 · 가설 · 판정 규칙이 그 폴더의 `README.md`에 있다
