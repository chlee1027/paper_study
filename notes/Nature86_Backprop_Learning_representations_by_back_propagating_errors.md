...
| 돌려 본 범위 | **2026년 9월 20일에 식 (1)-(9)를 구현해 돌렸다.** 논문의 두 실험(대칭 검출 · 가족 트리)과, 논문이 숫자 없이 적은 두 문장을 재는 실험 넷이다. 본문에서 **「돌려 본 것」**으로 시작하는 문단이 그 결과이고, 나머지는 읽어서 적은 것이다 |
...
### 돌려 본 것 — 그림 1은 유일한 해가 아니라 풀리면 늘 나오는 모양이다

논문의 설정 그대로 씨앗만 50개로 바꿔 돌렸다. 「풀렸다」의 기준은 논문이 가족 트리에서 쓴 것(켜져야 할 출력 0.8 위, 꺼져야 할 출력 0.2 아래)을 빌려 왔다 — **논문은 대칭 검출의 수렴 기준을 적지 않았다.**

| 재는 값 | 결과 |
|---|---|
| 푼 씨앗 | 50개 중 **46개** |
| 걸린 훑기 | 중앙값 **2,280** (최소 708, 최대 17,751) |
| 부호 반대칭 | 마주 보는 두 가중치의 어긋남이 **가장 큰 씨앗에서도 0.0007** (0이면 완전한 반대칭) |
| 한쪽 절반 세 크기의 비 | 중앙값 **1.00 : 1.98 : 3.94** |
| 못 푼 4개의 오차 | 넷 다 **정확히 4.0** |

**세 가지가 따라 나온다.**

**하나. 1 : 2 : 4는 이 난수 씨앗의 우연이 아니다.** 푼 씨앗 46개가 모두 같은 비와 같은 부호 대칭에 닿았다. 그림 8의 가중치는 **이 과제가 가진 모양**이고, 논문이 한 번의 결과만 보인 것이 문제가 되지 않는다.

**둘. 1,425는 폭 안의 한 점이다.** 훑기 수가 708에서 17,751까지 벌어진다. 논문이 적은 한 숫자를 「이 과제는 1,425번이면 된다」로 읽으면 안 된다.

**셋. 못 푼 넷은 느렸던 것이 아니라 갇힌 것이다.** 대칭인 입력이 8개이므로 **늘 0만 내는 해의 오차가 정확히 $0.5 \times 8 = 4.0$**이다. 넷의 오차가 모두 그 값이었다. 상한을 늘려도 이 넷은 풀리지 않는다.
...
**트리가 두 개인 것이 핵심 장치다.** 영국 쪽 열두 명과 이탈리아 쪽 열두 명이 **이름만 다르고 관계 구조가 똑같다.** 사람은 모두 24명이다.

**돌려 본 것 — 논문의 「104개」가 무엇을 센 것인지.** 두 집안을 코드로 세워 관계 열두 가지를 전부 펼쳐 세었더니 수가 갈렸다.

| 센 것 | 개수 |
|---|---|
| ⟨사람 1⟩ ⟨관계⟩ ⟨사람 2⟩ 세 낱말 조합 | **112** |
| ⟨사람 1⟩ ⟨관계⟩ 짝, 곧 망에 넣는 입력 패턴 | **104** |
| 답이 둘인 짝 | **8** |

**논문의 104는 세 낱말 조합이 아니라 입력 패턴의 수다.** 차이 8은 답이 둘인 짝이고, Colin · Charlotte과 이탈리아 쪽의 Alfonso · Sophia가 각각 고모와 삼촌을 둘씩 가져 정확히 여덟이 된다. 논문이 그림 3에서 「Colin은 고모가 둘이라 정답이 둘」이라고 적은 것과 맞는다.
...
**돌려 본 것 — 논문의 설정값 그대로로는 학습되지 않았다.** 위의 설정(ε = 0.01, α = 0.9, 1,500 훑기, 감쇠 0.2%, 오차 판정에 여유)을 그대로 두고 씨앗 열 개로 돌렸더니, 학습에 쓴 100개 중 **24개 출력이 모두 0.8 위 · 0.2 아래인 예가 0개**였다. 가장 활성이 높은 출력이 정답과 같은지로 느슨하게 세어도 100개 중 6.2개다. **망이 「전부 꺼짐」 자리에 앉아 거기서 나오지 못했다.**

ε만 올리면 학습된다. 돌리기 전에 정해 둔 격자(ε 넷 × 감쇠 둘 × 오차 0 처리 둘)에서 **ε을 0.05에서 0.10 사이로 두면 느슨한 기준으로 100개 중 91에서 97개**까지 올라간다.

**그러므로 논문의 ε = 0.01과 이 구현 사이에 적히지 않은 차이가 있다.** 무엇인지는 이번 실행으로 답이 나오지 않았다. 지금 의심하는 것은 오차 0 처리를 기울기의 어느 시점에 먹이는지와, 예 100개의 기울기를 합으로 쓰는지 평균으로 쓰는지다. **둘 다 논문에 적혀 있지 않다.**

**미학습 4개는 논문만큼 맞히지 못했다.** 가장 잘 학습된 칸에서도 느슨한 기준으로 **32번 중 17번**(씨앗 여덟 개 × 4개)이다. 논문은 4개를 모두 맞혔다고 적었다. **4개는 작은 수라 한 번의 결과로는 폭을 알 수 없다**는 것이 이 실행이 보인 것이다.
...
**돌려 본 것 — 대칭 검출에서는 반대 방향으로 나왔다.** 위 3번과 4번이 말하는 것이 곧 실험이다. 은닉 유닛 수만 바꿔 씨앗 50개씩 돌렸다.

| 은닉 유닛 | 가중치 수 | 푼 씨앗 | 못 푼 비율 | 그중 늘 0을 내는 해 |
|---|---|---|---|---|
| 2 | 17 | 46 / 50 | **0.08** | 4 |
| 3 | 25 | 34 / 50 | **0.32** | 16 |
| 4 | 33 | 37 / 50 | **0.26** | 13 |
...
**연결을 더 둘수록 갇히는 비율이 올라갔다.** 그리고 **못 푼 것이 하나도 빠짐없이 오차 4.0인 「늘 0을 내는 해」**였다 — 오차가 작은데 기준에만 못 미친 씨앗은 네 조건 모두 0개다. 상한 20,000 훑기가 짧아서가 아니라 갇힌 것이다.

**논문의 서술을 뒤집는 것으로 읽지는 않는다.** 논문은 「여러 과제에서」라고 적었고 이것은 과제 하나다. 적을 수 있는 것은 여기까지다 — **이 과제 이 격자에서는 연결을 늘리자 갇히는 비율이 0.08에서 0.56으로 올라갔다.** 논문의 그 문단에 숫자가 없으므로 어느 쪽이 더 흔한지는 여전히 모른다.

**같은 절차인데 과제가 다르면 방향도 다르다.** 퍼셉트론이 못 풀던 배타적 논리합을 같은 망으로 돌리면 은닉 2개에서 100번 중 95번, 4개에서 100번 중 100번 풀린다. **여기서는 유닛을 늘리는 것이 도움이 되었다.**
...
| **깊이가 깊어질 때 기울기가 줄어드는 문제를 다루지 않는다** | **돌려 본 것이다.** 은닉층을 1개에서 7개까지 두고 학습 시작 시점의 층별 $\partial E/\partial w$ 크기를 쟀더니, 깊이 3 이상에서 **한 층마다 10배에서 12배씩** 줄었다. 은닉 7개 망에서는 맨 아래 층이 맨 위 층의 **1,200만분의 1**이다 | 논문의 망은 최대 다섯 층이라 이 문제가 드러날 깊이가 아니었다. 실제 배수가 $y(1-y)$의 최댓값 0.25보다 가파른 것은 **가중치 크기까지 함께 곱해지기** 때문이다. 이 문제가 정리되는 것은 뒤의 일이다 |
...
**넷. 돌려 보면 「논문이 적지 않은 것」이 먼저 드러난다.** 식 (1)-(9)를 구현하고 나서 막힌 자리는 식이 아니라 **적혀 있지 않은 설정**이었다. 대칭 검출은 수렴 기준이 없어 가족 트리 쪽 기준을 빌려 와야 했고, 가족 트리는 처음 가중치 범위와 시험 집합 고르는 법이 없어 내가 정해야 했다. **그 셋을 밝히지 않으면 재현이라고 부를 수 없다.** 논문을 읽을 때 「무엇이 적혀 있는가」만큼 「무엇이 빠져 있는가」를 적어 두는 이유가 여기에 있다.

**다섯. 지금 확인된 것은 여기까지다.** 원문 네 쪽을 읽었고, 식 (1)-(9)를 구현해 돌렸다. 그림 8과 9의 수치는 논문의 가중치를 식에 넣어 계산한 것이고, 본문에서 **「돌려 본 것」**으로 시작하는 문단이 실제로 돌린 결과다. **되먹임 있는 망(9절)은 구현하지 않았고, 은닉 유닛의 세 번째 축(그림 12의 유닛 6, 집안의 어느 갈래)도 재지 않았다.**

---

## 17. 물어 두었던 것과 그 답

읽고 나서 네 가지를 물어 두었고, 돌려 보고 셋에 답이 섰다. **답이 선 것은 위 절들에 숫자로 옮겨 적었고, 여기에는 물음과 결론만 한 줄씩 남긴다.**

| 물어 두었던 것 | 지금 말할 수 있는 것 |
|---|---|
| 1 : 2 : 4 구조는 항상 나오는가, 이 난수 씨앗에서만 나온 해인가 | **답이 섰다.** 씨앗 50개 중 푼 46개가 모두 같은 비(중앙값 1.00 : 1.98 : 3.94)와 같은 부호 대칭에 닿았다. 씨앗의 우연이 아니다 |
| 미학습 4개를 맞힌 것은 얼마나 재현되는가 | **답이 섰고, 논문만큼은 아니었다.** 가장 잘 학습된 설정에서도 32번 중 17번이다. 다만 **논문의 설정값 그대로로는 학습 자체가 되지 않아** 다른 학습률에서 잰 값이라, 논문의 4/4와 나란히 놓을 수 없다 |
| 가중치 감쇠가 없으면 그림 12의 축이 덜 읽히는가 | **순서를 매기지 못했다.** 감쇠를 켠 쪽의 국적 축 또렷함이 중앙값 0.775, 끈 쪽이 0.651로 켠 쪽이 높지만 **두 조건의 범위가 겹친다**(켬 0.437~0.909, 끔 0.486~0.886). 씨앗 사이의 변동이 조건 사이의 차이보다 크다 |
| 연결을 몇 개 더 두면 우회로가 생기는가 | **답이 섰고, 반대 방향이었다.** 11.1절에 적었다 — 이 과제에서는 은닉 유닛을 2개에서 6개로 늘리자 갇히는 비율이 0.08에서 0.56으로 올라갔다 |

**새로 생긴 물음 셋.**

- **논문의 ε = 0.01과 이 구현의 차이가 무엇인가?** 같은 망 · 같은 식인데 1,500 훑기에 학습이 되지 않는다. 오차 0 처리를 기울기의 어느 시점에 먹이는지, 예 100개의 기울기를 합으로 쓰는지 평균으로 쓰는지가 후보이고 **둘 다 논문에 적혀 있지 않다.** 같은 해에 나온 긴 판(참고 문헌 5번)에 적혀 있을 만한 자리다.
- **은닉 유닛을 늘릴 때 갇히는 비율이 올라간 것이 이 과제만의 성질인가?** 같은 절차로 배타적 논리합을 돌리면 유닛을 늘릴수록 잘 풀린다(2개 95/100, 4개 100/100). **과제에 따라 방향이 갈린다면 무엇이 그 방향을 정하는가.**
- **깊이마다 10배에서 12배씩 줄어드는 것을 무엇이 막는가?** 이 배수는 학습 시작 시점의 것이고, 학습이 진행되며 어떻게 변하는지는 재지 않았다.
...
- **「돌려 본 것」 문단들의 근거**: `experiments/backprop_1986/` — 노트북과 결과 CSV 아홉 개, 그리고 돌리기 전에 적은 질문 · 가설 · 판정 규칙이 그 폴더의 `README.md`에 있다
- 배타적 논리합과 퍼셉트론 쪽 수치의 근거: `experiments/perceptron_1958/`
